...
while {_units_near > 0} do {
    _units_near = [markerPos _sector, 300, GRLIB_side_friendly] call F_getUnitsCount;
    {
        if (((damage _x) < 0.5) && !(_x in _healed_civs)) then {
            ...
            private _civ = _x;
            [objNull] params ["_medic"];
            // Awaiting 'ACE_medic' variable to be relayed from client side
            waitUntil {
                _medic = _civ getVariable ["ACE_medic",objNull];
                !isNull _medic;
            };
            // Plus this sequence of events is a bit more sensible
            _healed_civs pushBack _civ;
            stats_civilians_healed = stats_civilians_healed +1;
            [4, [_civ, _medic]] remoteExec ["F_cr_globalMsg"];
