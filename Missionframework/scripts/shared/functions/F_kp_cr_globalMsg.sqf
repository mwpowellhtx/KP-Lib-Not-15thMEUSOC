...
switch (_msgType) do {
	...
	case 2: {systemChat (format [localize "STR_CR_KILLMSG", (name (_data select 0), name (_data select 1))]);};
	case 3: {systemChat (format [localize "STR_CR_RESISTANCE_KILLMSG", (name (_data select 0), name (_data select 1))]);};
	case 4: {systemChat (format [localize "STR_CR_HEALMSG", name (_data select 0), name (_data select 1)]);};
